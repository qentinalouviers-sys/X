...
export function securityHeaders({ secure }) {
  const headers = {
    ...
    // 'no-referrer' pousse Chrome à envoyer `Origin: null` sur les POST de
    // formulaire, ce qui casse la vérification anti-CSRF. 'same-origin' donne
    // la même confidentialité vis-à-vis des tiers — la CSP interdit de toute
    // façon la moindre requête sortante — tout en gardant un Origin exploitable.
